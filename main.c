...
double aspectRatio;
double vFOV;
Vec3 cameraLocation = {{-0, 110, 0, 121}};
...
static uint16_t setGlobalVariables(char** argv) {
...
int main(int argc, char** argv) {
    ...
    srand(time(0));
    uint16_t test = setGlobalVariables(argv);
    ...
    for (int i = 0; i < 360; i++) {
        cameraDirection.arr[0] = cos(250 * PI / 180);
        cameraDirection.arr[1] = sin(250 * PI / 180);
        cameraDirection.arr[2] = 0.5 * sin(i * PI / 180);
        ...
        for (int j = 0; j < num; j++) {
            ...
            Sphere* sphere = makeSphere(center, 1 + 5 /(j * 0.4 + 1), j % 2 == 0 ? red : green, NULL, 0.0, 0.0, 1.2);
            spheres[j] = sphere;
        }
